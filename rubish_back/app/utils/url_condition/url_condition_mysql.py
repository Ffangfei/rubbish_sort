...
class UrlCondition(object):
    def __init__(self, url_args):  # 根据给予的url_args进行格式化的处理
        ...
        for key, value in url_args.items():
            ...
            for v in value:
                if key == '_per_page' or key == '_page':
                    ...
                    v = v.replace(' ', '')
                    order_list = [
                        str(item_order) for item_order in v.split(',')
                    ]
                elif key == '_limit':
        ...
        if len(order_list) == len(sort_list):
            self.sort_limit_dict['_sort_dict'] = dict(
                zip(sort_list, order_list))
        else:
            self.sort_limit_dict['_sort_dict'] = dict(
                zip(sort_list, ['desc' for i in range(len(sort_list))]))

    @classmethod
    def init_filter_dict(cls, filter_list):  # 初始化filter_dict供 后续函数使用
        filter_dict = dict()
...
def filter_query(query, filter_map, table):  # 根据filter条件生成相应语句
...
def sort_limit_query(query, sort_limit_dict, table):  # 根据sort和limit条件生成相应的语句
...
def page_query(query, page_dict):  # 根据分页信息生成相应语句
    if '_page' not in page_dict or '_per_page' not in page_dict:
        data = query.all()
        return data, len(data)
    page = int(page_dict['_page'])
    per_page = int(page_dict['_per_page'])
    pagination = query.paginate(page=int(page),
                                per_page=int(per_page),
                                error_out=False)
    return pagination.items, pagination.total


def process_query(query, filter_dict, sort_limit_dict,
                  table):  # 调用上述函数 生成最终的语句
    query = filter_query(query, filter_dict, table)
    query = sort_limit_query(query, sort_limit_dict, table)
    return query


def count_query(query, filter_dict, sort_limit_dict,
                table):  # 调用上述函数 生成获取查询数量的语句
    query = filter_query(query, filter_dict, table)
    query = sort_limit_query(query, sort_limit_dict, table)
    return query.count()
